...
portTickType onTimeout = 0;
portTickType reboundLightTimeout = 0;
...
int isLightInterruptorClicked(fsm_t*);

void alarm_on(fsm_t*);
void alarm_off(fsm_t*);
void led_on(fsm_t*);
void led_off(fsm_t*);
...
struct fsm_trans_t trans_alarm[] = {
  ...
  {ALARM_ON, checkCorrectCodeFlag, ALARM_OFF, alarm_off},
  {ALARM_ON, isSomeoneHere, ALARM_ON, alarm_on},
  {-1, NULL, -1, NULL},
};
...
struct fsm_trans_t interruptor[] = {
  {LED_OFF, isLightInterruptorClicked, LED_ON, led_on},
  {LED_ON, isLightInterruptorClicked, LED_ON, led_on},
  {LED_ON, checkTimeoutLight, LED_OFF, led_off},
  {-1, NULL, -1, NULL},
};
...
void run(void* ignore)
{
    ...
    gpio_pin_intr_state_set(BUTTON_D2, GPIO_PIN_INTR_NEGEDGE);
...
int isLightInterruptorClicked(fsm_t* this) {
  return lightButtonPressed;
}

int checkTimeoutLight(fsm_t* this) {
...
void isr_gpio() {
  ...
  if (status & BIT(BUTTON_D3)) {
    ...
    isPresent = true;
  } else if (status & BIT(BUTTON_D2)) {
    if (now > reboundLightTimeout) {
      reboundLightTimeout = now + REBOUND_TIME;
