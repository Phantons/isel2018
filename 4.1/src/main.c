...
struct fsm_trans_t interruptor[] = {
  ...
  {LED_ON, checkIfPressed, LED_OFF, led_off},
  {-1, NULL, -1, NULL},
};
...
int checkIfPressed(fsm_t* this) {
  return ((!GPIO_INPUT_GET(BUTTON_D3) || GPIO_INPUT_GET(BUTTON_D8)) && getCurrentTime() > timeout);
}
...
void led_on (fsm_t* this) {
  timeout = getCurrentTime() + REBOUND_TIME;
